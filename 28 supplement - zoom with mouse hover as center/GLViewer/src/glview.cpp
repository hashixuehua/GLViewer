...
GLView::GLView(QWidget *parent)
    : QOpenGLWidget{ parent }, m_lightPos(5.f, 20.f, 5.0f), mSelectionFramebuffer(nullptr), mMSAAFramebuffer(nullptr)
{
    ...
    //  render
    auto f3 = std::bind(&GLView::OnMSAAModeChanged, this, std::placeholders::_1, std::placeholders::_2);
    auto f4 = std::bind(&GLView::OnEdgeLineModeChanged, this, std::placeholders::_1, std::placeholders::_2);
    ViewerSetting::OnMSAAModelChanged = f3;
    ViewerSetting::OnEdgeLineModeChanged = f4;
    renderSettingDlg = new RenderSettingDialog(this);
...
bool GLView::tryToExcuteCommandOnValidKeyPress()
{
    ...
    {
        QVector3D hitPt;
        if (getRayIntersection(ViewerSetting::currentMousePos[0], ViewerSetting::currentMousePos[1], hitPt))
        {
...
{
    ViewerSetting::currentMousePos[0] = event->x();
    ViewerSetting::currentMousePos[1] = event->y();
...
void GLView::renderSetting(void)
{
    renderSettingDlg->exec();
}
...
void GLView::OnMSAAModeChanged(bool enable, int mode)
{
    if (!enable)
        Setting::sampleSieOfMSAA = -1;
    else if (mode == 0)
        Setting::sampleSieOfMSAA = 4;
    else if (mode == 1)
        Setting::sampleSieOfMSAA = 8;
    else if (mode == 2)
        Setting::sampleSieOfMSAA = 16;
    else
        return;

    MSAAResize(Setting::devicePixelRatio * m_camera.SCR_WIDTH, Setting::devicePixelRatio * m_camera.SCR_HEIGHT);
    update();
}

void GLView::OnEdgeLineModeChanged(bool showEdgeLine, bool PPAA)
{
    ViewerSetting::showEdgeLine = showEdgeLine;
    ViewerSetting::edgeLinePPAA = PPAA;
    update();
}
