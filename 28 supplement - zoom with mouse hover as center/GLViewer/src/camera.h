...
public:
    ...
    float GetDepth(int px, int py)
    {
        ...
        glFunc->glReadPixels(px * Setting::devicePixelRatio /** device_ratio*/, (h - py) * Setting::devicePixelRatio/* * device_ratio*/, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &zpos);
        //zpos = zpos * 2 - 1.0;
        return zpos;
    }

    void GetScenePoint(float px, float py, float depth, QVector3D& result)
    {
        QMatrix4x4 project;
        project.perspective(this->Zoom, (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);

        QVector4D wcsPt = Screen2Scene({ px, SCR_HEIGHT - py }, { 0, 0, SCR_WIDTH, SCR_HEIGHT }, this->GetViewMatrix(),
            project, &depth);

        result = wcsPt.toVector3D();
    }

    bool GetScenePoint(float px, float py, QVector3D& result, float& depth)
    {
        depth = GetDepth(px, py);
        result.setZ(depth);

        if (abs(result.z() - 1.0f) < 1e-9)
        {
            result = { 0.f, 0.f, 0.f };
            return false;
        }
    ...
    void ProcessMouseLeftButtonPress(QMouseEvent* event)
    {
        ...
        if (firstMouse)
        {
            ...
            lastY = ypos;
            float depth;
            GetScenePoint(xpos, ypos, center, depth);
