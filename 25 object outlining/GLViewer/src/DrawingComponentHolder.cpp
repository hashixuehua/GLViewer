    ...
    if (ViewerSetting::showModel)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
        {
            ...
            if (drawScaled)
            {
                ...
                //  TODO 后续进行统一参数设置管理
                float xScale = std::min(1.f + 100.f / vecCross.X, 2.f);
                float yScale = std::min(1.f + 100.f / vecCross.Y, 2.f);
                float zScale = std::min(1.f + 100.f / vecCross.Z, 2.f);
                ...
                mat = ViewerSetting::matirxYup2Zup * mat;
