...
public:
    ...
    DrawingLinesHolder(QOpenGLFunctions_4_5_Core* glFunc) : m_glFunc(glFunc)
    ...
    {
        modelNodes.push_back(ModelNode(GLNodeType::GLCurveNode0, line.pt0, -1));
        modelNodes.push_back(ModelNode(GLNodeType::GLCurveNode1, line.pt1, -1));
        modelNodes.push_back(ModelNode(GLNodeType::GLCurveNodeM, 0.5 * (line.pt0 + line.pt1), -1));
    }
    ...
    void addModelNode(GLCurveType cType, const list<Line>& lstLine)
    {
        ...
        {
            addLineModelNode(*lstLine.begin());
        }
        else if (cType == GLCurveType::GLArc || cType == GLCurveType::GLCircle)
        {
            ...
            modelNodes.push_back(ModelNode(GLNodeType::GLCurveNode0, lstLine.begin()->pt0, -1));
            modelNodes.push_back(ModelNode(GLNodeType::GLCurveNode1, lstLine.back().pt1, -1));
            modelNodes.push_back(ModelNode(GLNodeType::GLCurveNodeM, itrItem->pt0, -1));
        }
    ...
    void pushCurveData(GLCurveType curveType, const list<Line>& lstLine)
    {
        ...
        addModelNode(curveType, lstLine);
    }
    ...
    void updateLinesByInputFileData(const vector<string>& layerName, const Vector3f& normal, const list<Line>& lstLine)
    {
        ...
        for (auto& item : lstLine)
        {
            ...
            temp.back().pt0 = pt0;
            temp.back().pt1 = pt1;
            addModelNode(GLCurveType::GLLine, temp);
        }
    ...
    void DrawLines(QOpenGLShaderProgram& shader, const PeviewData& previewData)
    {
        //  sort by layer
    ...
    void updateLinesPreview(const PeviewData& previewData, vector<float>& lineVerts, vector<unsigned int>& lineInds)
    {
