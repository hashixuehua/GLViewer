...
void Model::DrawRectSelect(QOpenGLShaderProgram& shader, const vector<Vector3f>& rectData)
{
    drawingViewerElemHolder->drawRectSelect(shader, rectData);
}

void Model::DrawCopyElementsPreview(QOpenGLShaderProgram& shader)
{
    //  实例化的思路，复用之前的数据，设置对应的transform
    //if (showModel)
    {
        drawingComponentHolder->drawCopyElementsPreview(shader);
    }

    //if (showLines)
    {
        drawingLinesholder->DrawCopyElementsPreview(shader);
    }
}
...
void Model::AddVertexToLines(const CommandPara& para)
{
    ...
    if (ViewerSetting::previewData.curveType == GLCurveType::GLTemp && ViewerSetting::viewerStatus == ViewerStatus::Copy && !ViewerSetting::basePt4Copy)
    {
        ViewerSetting::basePt4Copy = new Vector3f(ViewerSetting::previewData.curData.back().pValue);
    }
}
...
bool Model::copyElement(const list<Line>& lstLine)
{
    //  copy等只需要预览的，在“画线”操作完成后执行copy
    if (ViewerSetting::previewData.curveType == GLCurveType::GLTemp && ViewerSetting::viewerStatus == ViewerStatus::Copy)
    {
        if (!ViewerSetting::basePt4Copy)
            ViewerSetting::basePt4Copy = new Vector3f(lstLine.back().pt0);

        Vector3f offset = lstLine.back().pt1 - *ViewerSetting::basePt4Copy;
        drawingLinesholder->CopySelectedData(offset);
        drawingComponentHolder->copySelectedData(offset);
        return true;
    }
...
{
    list<pair<int, const BodyInfo*>> lstBody;
    drawingComponentHolder->getSelectedBody(lstBody);

    Vector3f planePt, planeNormal;
    float offset;
    if (lstBody.size() != 1 || !CGLibUtils::getClickHittedPlane(rayOri, rayEnd, *lstBody.back().second, planePt, planeNormal))
    {
...
void Model::ClearSelected()
{
    ...
    drawingLinesholder->clearSelected();
}

void Model::UpdateSelectInfo(const Body& rectData)
{
    drawingLinesholder->UpdateSelectInfo(rectData);
}

void Model::DeleteSelectedElements()
{
    drawingLinesholder->deleteSelectedLines();
    drawingComponentHolder->deleteSelectedComponents();
}

void Model::DeleteLinesByRemovedLayer(const LayerData& layerData)
{
    drawingLinesholder->deleteLinesByRemovedLayer(layerData);
}

void Model::readLinesFromFile(const wstring& fileName)
{
    drawingLinesholder->readLinesFromFile(fileName);
}

void Model::saveLinesToFile(const wstring& fileName)
{
    drawingLinesholder->saveLinesToFile(fileName);
}
