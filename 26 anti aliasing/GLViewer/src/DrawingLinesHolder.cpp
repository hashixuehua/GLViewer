...
void DrawingLinesHolder::pushCurveData(GLCurveType curveType, const list<Line>& lstLine)
{
        ...
        curveType != GLCurveType::GLPoint)
    {
        ...
        target->layerName = LayerCache::getCurrentLayer().name;
    }
...
void DrawingLinesHolder::readLinesFromFile(const wstring& fileName)
{
    list<pair<string, list<pair<Vector3f, list<Line>>>>> lstNormal2Lines;
    ReadAndWriteUtils::DeserializeFromJson(fileName, lstNormal2Lines);

    for (auto& pair : lstNormal2Lines)
    {
        for (auto& normal2Lines : pair.second)
        {
            updateLinesByInputFileData(vector<string>{pair.first}, normal2Lines.first, normal2Lines.second);
        }
    }
}

void DrawingLinesHolder::saveLinesToFile(const wstring& fileName)
{
    ReadAndWriteUtils::SerializeToJson(lines, fileName);
}